...
function orchestrate(spec, token) {
  ...
  return runDocumentDownloader(spec.url, tempLocation)(spec)
    ...
    .then(function (state) {
      return runTokenChecker(state.metadata.get('latestVersion'), spec.url)(state);
    })
    .then(runThirdPartyChecker(httpLocation))
    .then(function (state) {
      return runPublisher(state.metadata)(state);
    })
    .then(function (state) {
      return runTrInstaller(state.metadata.get('thisVersion'), tempLocation)(state);
    })
    .then(function (state) {
      return runShortlink(state.metadata.get('thisVersion'))(state);
    })
    .then(function (state) {
      return finishTasks(state.metadata.get('thisVersion'), resultLocation)(state);
    })
    .catch(function (err) {
