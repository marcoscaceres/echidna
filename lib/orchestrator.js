...
function Job() {
  if (typeof this !== 'object') throw new TypeError('Job must be constructed via new');
  ...
  return function (state) {
    ...
    return DocumentDownloader.fetchAndInstall(url, tempLocation)
      .then(function () {
        ...
        state.history = state.history.add('The document could not be retrieved.');
...
Orchestrator.prototype.runSpecberus = function (httpLocation) {
  return function (state) {
    if (state.jobs['specberus'].status !== 'pending') return state;
    ...
    return SpecberusWrapper.validate(httpLocation)
      .then(function (report) {
        ...
        state.history = state.history.add('An error occurred while running Specberus.');
...
Orchestrator.prototype.runTokenChecker = function (latestVersion, url, token) {
  return function (state) {
    if (state.jobs['token-checker'].status !== 'pending') return state;
    ...
    return TokenChecker.check(latestVersion, token)
      .then(function (authReport) {
        ...
        if(authReport.authorized && matchSource) {
          ...
          state.history = state.history.add('You are not authorized to publish');
        ...
        state.history = state.history.add('An error occurred while running the Token Checker.');
    ...
    return ThirdPartyChecker.check(httpLocation)
      .then(function (extResources) {
        if (extResources.length === 0) {
          ...
          state.history = state.history.add('The document passed the third party checker.');

          return state;
        }
        else {
          state.history = state.history.add('The document contains non-authorized resources');
        ...
        state.history = state.history.add('An error occurred while running the Third Party Resources Checker.');
    ...
    return new Publisher(pubsystemService).publish(metadata)
      .then(function (errors) {
        if (errors.isEmpty()) {
          ...
          state.history = state.history.add('The document could not be published: ' + errors.map(function (error) {
        ...
        state.history = state.history.add('The document could not be published: ' + error.message);
  ...
  return function (state) {
    ...
    return trInstaller(tempLocation, finalTRpath)
      .then(function () {
        ...
        state.history = state.history.add('An error occurred while installing the document in TR');
  ...
  return function (state) {
    ...
    return updateTrShortlink(thisVersion)
      .then(function () {
        ...
        state.history = state.history.add('An error occurred while updating the shortlink.');
...
Orchestrator.prototype.finishTasks = function (thisVersion, resultLocation) {
  return function (state) {
    var cmd = global.SENDMAIL + ' SUCCESS ' + global.MAILING_LIST + ' ' + thisVersion;
...
Orchestrator.prototype.markStarted = function (jobName, previousJobName) {
  return function (state) {
    if (previousJobName === undefined || state.jobs[previousJobName].status === 'ok') {
...
Orchestrator.prototype.run = function (spec, token) {
  ...
  return Promise.resolve(self.markStarted('retrieve-resources')(spec))
    ...
    .then(function (state) {
      ...
      return Promise.resolve(self.markStarted('token-checker', 'specberus')(state))
        .then(self.runTokenChecker(metadata.get('latestVersion'), spec.url, token))
